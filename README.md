...
- **`FileManager/pytest.ini` → `--cov-fail-under`.** Currently `82`, against a measured 82.77%, having ratcheted 29 → 32 → 49 → 50 → 53 → 62 → 71 → 82. `FileManager/docs/REQUIREMENTS.txt` asks for 95%. Raise the floor in the same PR that raises real coverage.
